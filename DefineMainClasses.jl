...
#        groupby(_, :DeliveryHour) |>
        combine(_, :Price => (x -> quantile(x, 0.9)) => :iLastDecile,
                    ...
                    :Price => (x -> quantile(x, 0.7)) => :i55Quantile,
                    :Price => (x -> quantile(x, 0.3)) => :i45Quantile)
...
function GetBrain(cPolicyOutputLayerType, iDimState; β = 0.999, ηₚ = 0.0001, ηᵥ = 0.001)
    ...
    return Brain(β, 256, 100_000, 20_000, [], policy_net, value_net, ηₚ, ηᵥ, cPolicyOutputLayerType)
end
